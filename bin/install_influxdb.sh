...
# 2. Install Python dependencies: pandas, influxdb_client.
# 3. Clone a Git repository.
# 4. Change into the cloned repository's subdirectory.
# 5. Install a Helm release for InfluxDB.
# 6. Wait for its pod to be ready.
# 7. Fetch and decode the admin user token.
# 8. Create an InfluxDB bucket named 'UEData'.
...
GIT_REPO_URL="https://gerrit.o-ran-sc.org/r/ric-app/qp"
GIT_BRANCH="f-release"
REPO_DIR_NAME="qp" # The name of the directory git will clone into
TARGET_SUBDIR="qp/qp" # The subdirectory to cd into after cloning
...
install_python_packages() {
  echo "Installing Python packages: $PYTHON_PACKAGES (requires sudo for pip3)..."
...
clone_repository() {
  echo "Checking if repository directory '$REPO_DIR_NAME' already exists..."
  if [ -d "$REPO_DIR_NAME" ]; then
    echo "Repository directory '$REPO_DIR_NAME' already exists. Skipping clone."
  else
    ...
    if [ $? -ne 0 ]; then
      ...
      echo "Please check your Git configuration, network access, and repository URL."
      exit 1
    else
      echo "Repository cloned successfully into '$REPO_DIR_NAME'."
    fi
  fi
}
...
# 1. Ensure pip3 and Python dependencies are installed
install_pip3_if_needed
install_python_packages
...
# 3. Change into the target subdirectory
echo "Changing directory to $TARGET_SUBDIR..."
if [ ! -d "$TARGET_SUBDIR" ]; then
    echo "Error: Target subdirectory '$TARGET_SUBDIR' not found after cloning."
    echo "Please check the repository structure."
    exit 1
fi
cd "$TARGET_SUBDIR"
if [ $? -ne 0 ]; then
    echo "Error: Failed to change directory to $TARGET_SUBDIR."
    exit 1
fi
echo "Now in directory: $(pwd)"

echo "-----------------------------------------------------"
echo "Initial setup (Python, Git) complete. Proceeding with InfluxDB deployment."
echo "-----------------------------------------------------"

# 4. Helm and InfluxDB setup
...
while [ -z "$POD_NAME" ]; do
  ...
  # This command needs to run from the original directory or with full kubectl context,
  # as cd might affect relative paths for scripts if not careful.
  # kubectl commands are generally not affected by the current shell's CWD.
...
if kubectl exec "$POD_NAME" -- influx bucket create \
  ...
  echo "  - Python dependencies (pandas, influxdb_client) installed."
  echo "  - Git repository cloned and changed to $TARGET_SUBDIR."
  ...
  echo ""
  echo "InfluxDB Admin User Token (for reference):"
  echo "$INFLUXDB_TOKEN"
  echo "-----------------------------------------------------"
else
